...
namespace TeamVsTeam
{
    class Display
    {
            ...
            else
            {
                Console.Clear();
                Console.WriteLine("");
                Console.WriteLine("Looks like you're out of money!");
                Console.WriteLine("We require the minimum of $5 for the supplies cost.");
                Console.WriteLine("");
                Console.WriteLine("Press enter to continue.");
                Console.ReadLine();
            }
            ...
            //Look at team displays in the combatphase(), not combatphase(), the monsters rewrite in green always, split with setcursor? Should address Fix combat alignment due to growing combat characters as well
            //DisplayClass?

            //Hard - - - - - - :
            //Multiple actions per hero, RNG, growth dependent
            //Special Abilities (Unlockable from wearing equipment long enough? 5 stages, each progressively cooler thing since more skilled with that weapon. Encourages gear minded play.)
            ...
            public static void ShowCampMenu(out bool camping)
            {
                ...
                while (asking)
                {
                    ...
                    Console.WriteLine($"Current Funds: {Team.Funds:C2}");
                    Console.WriteLine($"Average Speed: {GetHeroSpeed():F2}");
                    Console.WriteLine($"Day #: {DayTracker.Day}");
